...
class TestDoublyLinkedListContains(unittest.TestCase):
    def setUp(self) -> None:
...
class TestDoublyLinkedListRemove(unittest.TestCase):
    def setUp(self) -> None:
        test_values = [5, 43, 92, 100, 4, 88, 23, 46]
        self.test_list = DoublyLinkedList()
        for val in test_values:
    ...
    def test_empty_list(self):
        new_test_list = DoublyLinkedList()
        self.assertFalse(new_test_list.remove(0))

class TestDoublyLinkedListTraverse(unittest.TestCase):
...
class TestDoublyLinkedListReverseTraverse(unittest.TestCase):
    def test_reverse_traverse(self):
        ...
        for val in reversed(test_value[1:]):
            with self.subTest(val=val):
